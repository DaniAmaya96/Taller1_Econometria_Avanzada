...
Para demostrar que $\hat\tau_{MCO}=\hat\tau_{DM}$.

Podemos tomar dos caminos, uno es utilizando el método de momentos y otro haciendo la minimización de cuadrados.

De la clase sabemos que el estimador de $MCO$ es el análogo muestral de los estimadores poblacionales de la $CEF$, entonces hagamos lo siguiente: tomemos el valor esperado de nuestra regresión.
...
$$
E[Y_i \mid D_i=1]= \beta_0+\tau
$$

Por lo tanto, si aplicamos la diferencia de medias:
...
Si tomamos el momento muestral, tendríamos que:
...
La segunda forma de llegar a la respuesta es aplicando la minimización de cuadrados a los residuos de nuestro modelo de regresión:
